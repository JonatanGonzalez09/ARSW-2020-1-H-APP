...
public interface UndergoesPersistence extends JpaRepository<Undergoes, Integer> {
	@Query(value = "select undergoes.* from patient p join stay on p.patient_id = stay.patient_id join undergoes on stay.stay_id = undergoes.stay_id join nurse on nurse.nurse_id = undergoes.nurse_id where nurse.nurse_id = :nurseId and DATE(undergoes.date) = select u.* from undergoes u join nurse on u.nurse_id = nurse.nurse_id where nurse.nurse_id = :nurseId and DATE(u.\"date\") = (select DATE(now()))", nativeQuery = true)
	...
	@Query(value= "select u.* from undergoes u join nurse on u.nurse_id = nurse.nurse_id where nurse.nurse_id = :nurseId and DATE(u.\"date\") = (select DATE(now()))", nativeQuery = true)
	...
	@Query(value= "select undergoes.* from \"procedure\" p join undergoes on p.procedure_id = undergoes.procedure_id join nurse on undergoes.nurse_id = nurse.nurse_id join on_call on nurse.nurse_id = on_call.nurse_id join usuarios on nurse.usuarios_user_id = usuarios.user_id where usuarios.gov_id = :nurseGovId and on_call.oncallend is null and done is null", nativeQuery = true)
	List<Undergoes> getUndergoesByNurseGovId(String nurseGovId);

	@Query(value= "select u.* from undergoes u join nurse on u.nurse_id = nurse.nurse_id where nurse.nurse_id = :nurseId and DATE(u.\"date\") = (select DATE(now())) and done is null", nativeQuery = true)
	List<Undergoes> getUndergoesNotDoneTodayByNurseId(int nurseId);
}